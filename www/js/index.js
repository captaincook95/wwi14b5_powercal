...
function startApp() {
						...
						if (((ui.prevPage[0].id == $("#new_appointment").prop(
							...
							ContactsOverviewSelectMode = true;
								...
								&& (((ui.toPage[0].id == $("#new_appointment")
										...
										"#appointment_details").prop("id"))) || ui.toPage[0].id == $(
										"#index").prop("id"))) {
							ContactsOverviewSelectMode = false;
						}

						if (ui.prevPage[0].id == $("#appointment_details")
								.prop("id")
								&& ui.toPage[0].id == $("#places_overview")
										.prop("id")) {
							PlacesOverviewUpdateMode = true;
						} else if (ui.prevPage[0].id == $("#places_overview")
								.prop("id")
								&& (ui.toPage[0].id == $("#appointment_details")
										.prop("id") || ui.toPage[0].id == $(
										"#index").prop("id"))) {
	...
	$("#deleteApp").on(
			'click',
			function() {
				if (confirm("Wollen Sie den Termin " + $('#titel_d').val()
						+ " wirklich löschen?") == true) {
	...
	$("#t_startd_d").on('click', function() {
		calendard($("#t_startd_d"));
	});

	$("#t_startt_d").on('click', function() {
		calendart($("#t_startt_d"));
	});

	$("#t_endd_d").on('click', function() {
		calendard($("#t_endd_d"));
	});

	$("#t_endt_d").on('click', function() {
		calendart($("#t_endt_d"));
	});
	...
	$(document).on('click', '#documentsList a', function() {
		...
		cordova.plugins.fileOpener2.open(filePath, 'image/jpeg', {
			error : function() {
				alert("Fehler");
			},
			success : function() {
...
function fillAppointmentsList() {
	getAppointments(function(tx, results) {
		...
		for (var i = 0; i < results.rows.length; i++) {
			...
			$("#appointments").append(
					'<li><a href="#" data-tid="' + row['tid'] + '">' + '<h2>'
							+ row['ANFANG'] + '</h2>' + '<p>' + row['TITEL']
							+ '</p>' + '</a>');
		}
...
function fillDetailsAppointmentForm(appointment) {
	...
	getAppointmentDetails(tid, function(tx, results) {
		...
		var date_start = row['ANFANG'];
		var date_end = row['ENDE'];
		var res = date_start.split(" ");
		$('#t_startd_d').val(res[0]);
		$('#t_startt_d').val(res[1]);
		res = date_end.split(" ");
		$('#t_endd_d').val(res[0]);
		$('#t_endt_d').val(res[1]);
	...
	getAppointmentContacts(tid, function(tx, results) {
		for (var i = 0; i < results.rows.length; i++) {
			...
			getContactDetails(kid, function(tx, results) {
				var row = results.rows.item(0); // Es kann immer nur eine Zeile
				// zurückkommen, da ID unique
				// ist
				$("#teilnehmer_d").append(
						'<li><a data-kid="' + row['kid'] + '">'
	...
	getAppointmentDocuments(tid, function(tx, results) {
		for (var i = 0; i < results.rows.length; i++) {
			...
			$("#documentsList_d").append(
					'<li><a data-path="' + file + '">' + bez + '</a></li>');
			$("#documentsList_d").listview("refresh");
		}
	});
}
...
function calendard(control) {
	...
	function onSuccess(date) {
		/*
		 * var time = date.getHours() + ':' + date.getMinutes(); var day =
		...
		if (bez == "") {
			...
			if (frompage == "#new_appointment") {
				$('#documentsList').append(
						'<li><a href="#" data-path="' + pathToFile + '">' + bez
								+ '</a></li>');
				$("#documentsList").listview("refresh");
			} else {
				$('#documentsList_d').append(
